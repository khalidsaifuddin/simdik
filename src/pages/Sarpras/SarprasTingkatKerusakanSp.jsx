...
class SarprasTingkatKerusakanSp extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapSarprasTingkatKerusakanSp(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/Sarpras/getRekapSarprasTingkatKerusakanSpExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    bukaPengaturan = () => {
        // alert('oke');
        this.props.setJudulKanan('Menu Kerusakan Per Sekolah');
    ...
    {
        return (
            <Page name="SarprasTingkatKerusakanSp" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <NavTitleLarge>
                        Kerusakan Sarpras {this.state.jenis ==='umum' ? 'Umum' : (this.state.jenis ==='lab' ? 'Laboratorium' : (this.state.jenis === 'lab_komputer' ? 'Lab Komputer' : 'Umum'))}
                    </NavTitleLarge>
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={"/Sarpras/TingkatKerusakan/"+this.state.jenis}>Per Wilayah</Button>
                ...
                <Navbar sliding={false}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sarpras/TingkatKerusakan/1/"+this.state.routeParams.kode_wilayah+ "/" + this.state.jenis : "/Sarpras/TingkatKerusakan/2/"+this.state.routeParams.kode_wilayah+ "/" + this.state.jenis)}>Per Wilayah</Button>
                            <Button tabLink="#tab2" tabLinkActive>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <div className="data-table-footer" style={{display:'block'}}>
                            <div className="data-table-pagination">
                                ...
                                <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.rekap_sarpras_tingkat_kerusakan_sp.total) ? "disabled" : "" )}>
                                    <i className="icon icon-next color-gray"></i>
                                </a>
                                <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.rekap_sarpras_tingkat_kerusakan_sp.total)} sekolah</span>
                            </div>
                        </div>
                        <table>
                            ...
                            :
                            <>
                            {this.props.rekap_sarpras_tingkat_kerusakan_sp.rows.map((option)=>{
                                return(
                                    <tr>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    {/* <MenuDropdownItem href={"/Sarpras/TingkatKerusakan/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapSarprasTingkatKerusakanSp: Actions.getRekapSarprasTingkatKerusakanSp,
...
function mapStateToProps({ App, Sarpras, Gtk, RaporDapodik, RekapSarpras }) {
    ...
    return {
        ...
        rekap_sarpras_tingkat_kerusakan_sp: RekapSarpras.rekap_sarpras_tingkat_kerusakan_sp
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(SarprasTingkatKerusakanSp));