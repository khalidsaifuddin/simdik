...
class GTKAgamaSp extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapGTKAgamaSp(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/GTK/getRekapGTKAgamaSpExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    bukaPengaturan = () => {
        // alert('oke');
        this.props.setJudulKanan('Menu Agama Per Sekolah');
    ...
    {
        return (
            <Page name="GTKAgamaSp" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <NavTitleLarge>
                        Agama GTK Per Sekolah
                    </NavTitleLarge>
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={"/GTK/Agama"}>Per Wilayah</Button>
                ...
                <Navbar sliding={false}>
                    <NavLeft >
                        <Link iconIos="f7:chevron_left" iconAurora="f7:chevron_left" iconMd="material:chevron_left" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/GTK/Agama" : "/GTK/Agama/1/"+this.state.routeParams.kode_wilayah.substring(0,2) + "0000" )}>Kembali</Link>
                    </NavLeft>
                    <NavTitle sliding>{this.props.wilayah.rows[0].nama}</NavTitle>
                    <NavTitleLarge>
                        Agama GTK
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/GTK/Agama/1/"+this.state.routeParams.kode_wilayah : "/GTK/Agama/2/"+this.state.routeParams.kode_wilayah)}>Per Wilayah</Button>
                            <Button tabLink="#tab2" tabLinkActive>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <div className="data-table-footer" style={{display:'block'}}>
                            <div className="data-table-pagination">
                                ...
                                <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.rekap_gtk_agama_sp.total) ? "disabled" : "" )}>
                                    <i className="icon icon-next color-gray"></i>
                                </a>
                                <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.rekap_gtk_agama_sp.total)} sekolah</span>
                            </div>
                        </div>
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    ...
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colspan="14">Agama</th>
                                </tr>
                                <tr>
                                    <th className="numeric-cell">Total</th>
                                    <th className="numeric-cell">Islam</th>
                                    <th className="numeric-cell">Kristen</th>
                                    <th className="numeric-cell">Katholik</th>
                                    <th className="numeric-cell">Hindu</th>
                                    <th className="numeric-cell">Budha</th>
                                    <th className="numeric-cell">Konghucu</th>
                                    <th className="numeric-cell">Kepercayaan</th>
                                    <th className="numeric-cell">Tidak Diisi</th>
                                </tr>
                            </thead>
                            <tbody>
                            {this.state.loading ?
                            ...
                            :
                            <>
                            {this.props.rekap_gtk_agama_sp.rows.map((option)=>{
                                return(
                                    <tr>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    {/* <MenuDropdownItem href={"/GTK/Agama/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_total))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_islam))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_kristen))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_katholik))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_hindu))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_budha))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_konghucu))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_kepercayaan))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.gtk_lainnya))}</td>
                                    </tr>
                                )
                            })}
                            </>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapGTKAgama: Actions.getRekapGTKAgama,
      getRekapGTKAgamaSp: Actions.getRekapGTKAgamaSp,
      setJudulKanan: Actions.setJudulKanan,
      setIsiKanan: Actions.setIsiKanan
    }, dispatch);
}

function mapStateToProps({ App, Gtk, RaporDapodik, RekapGTK }) {
    ...
    return {
        ...
        rekap_gtk_agama_sp: RekapGTK.rekap_gtk_agama_sp
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(GTKAgamaSp));