...
class PesertaDidikAgamaSp extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapPesertaDidikAgamaSp(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/PesertaDidik/getRekapPesertaDidikAgamaSpExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    {
        return (
            <Page name="PesertaDidikAgamaSp" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={"/PesertaDidik/Agama"}>Per Wilayah</Button>
                ...
                <Navbar sliding={false}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/PesertaDidik/Agama/1/"+this.state.routeParams.kode_wilayah : "/PesertaDidik/Agama/2/"+this.state.routeParams.kode_wilayah)}>Per Wilayah</Button>
                            <Button tabLink="#tab2" tabLinkActive>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <div className="data-table-footer" style={{display:'block'}}>
                            <div className="data-table-pagination">
                                ...
                                <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.rekap_peserta_didik_agama_sp.total) ? "disabled" : "" )}>
                                    <i className="icon icon-next color-gray"></i>
                                </a>
                                <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.rekap_peserta_didik_agama_sp.total)} sekolah</span>
                            </div>
                        </div>
                        <table>
                            ...
                            :
                            <>
                            {this.props.rekap_peserta_didik_agama_sp.rows.map((option)=>{
                                return(
                                    <tr>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_islam))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_kristen))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_katholik))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_hindu))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_budha))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_konghucu))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_kepercayaan))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pd_agama_tidak_diisi))}</td>
                                    </tr>
                                )
                            })}
                            </>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapPesertaDidikAgamaSp: Actions.getRekapPesertaDidikAgamaSp,
...
function mapStateToProps({ App, PesertaDidik, Gtk, RaporDapodik, RekapPesertaDidik }) {
    ...
    return {
        ...
        rekap_peserta_didik_agama_sp: RekapPesertaDidik.rekap_peserta_didik_agama_sp
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(PesertaDidikAgamaSp));