...
class PesertaDidikTingkatSp extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapPesertaDidikTingkatSp(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
    ...
    bukaPengaturan = () => {
        // alert('oke');
        this.props.setJudulKanan('Menu Tingkat Per Sekolah');
    ...
    {
        return (
            <Page name="PesertaDidikTingkatSp" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={"/PesertaDidik/Tingkat"}>Per Wilayah</Button>
                ...
                <Navbar sliding={false}>
                    <NavLeft >
                        <Link iconIos="f7:chevron_left" iconAurora="f7:chevron_left" iconMd="material:chevron_left" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/PesertaDidik/Tingkat" : "/PesertaDidik/Tingkat/1/"+this.state.routeParams.kode_wilayah.substring(0,2) + "0000" )}>Kembali</Link>
                    </NavLeft>
                    <NavTitle sliding>{this.props.wilayah.rows[0].nama}</NavTitle>
                    <NavTitleLarge>
                        Tingkat Peserta Didik
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/PesertaDidik/Tingkat/1/"+this.state.routeParams.kode_wilayah : "/PesertaDidik/Tingkat/2/"+this.state.routeParams.kode_wilayah)}>Per Wilayah</Button>
                            <Button tabLink="#tab2" tabLinkActive>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <div className="data-table-footer" style={{display:'block'}}>
                            <div className="data-table-pagination">
                                ...
                                <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.rekap_peserta_didik_tingkat_sp.total) ? "disabled" : "" )}>
                                    <i className="icon icon-next color-gray"></i>
                                </a>
                                <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.rekap_peserta_didik_tingkat_sp.total)} sekolah</span>
                            </div>
                        </div>
                        <table>
                            ...
                            :
                            <>
                            {this.props.rekap_peserta_didik_tingkat_sp.rows.map((option)=>{
                                return(
                                    <tr>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    {/* <MenuDropdownItem href={"/PesertaDidik/Tingkat/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapPesertaDidikTingkat: Actions.getRekapPesertaDidikTingkat,
      getRekapPesertaDidikTingkatSp: Actions.getRekapPesertaDidikTingkatSp,
...
function mapStateToProps({ App, PesertaDidik, Gtk, RaporDapodik, RekapPesertaDidik }) {
    ...
    return {
        ...
        rekap_peserta_didik_tingkat: RekapPesertaDidik.rekap_peserta_didik_tingkat,
        rekap_peserta_didik_tingkat_sp: RekapPesertaDidik.rekap_peserta_didik_tingkat_sp
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(PesertaDidikTingkatSp));