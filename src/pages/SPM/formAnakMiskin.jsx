...
import SpmReducer from '../../store/reducers/Spm.reducers';

class formAnakMiskin extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
    ...
    {
        return (
            <Page name="formAnakMiskin" hideBarsOnScroll>
                <Navbar sliding={false} large backLink="Kembali">
                    <NavTitle sliding className={this.state.loading ? "skeleton-text skeleton-effect-blink" : ""}>Peserta Didik Miskin</NavTitle>
                    <NavTitleLarge style={{background:'#F6BC31', color:'white'}} className={this.state.loading ? "skeleton-text skeleton-effect-blink" : ""}>Peserta Didik Miskin</NavTitleLarge>
                </Navbar>
                <Block strong style={{
                    marginTop:'0px', 
                    paddingLeft:'0px', 
                    ...
                    <div className="data-table-footer" style={{display:'block'}}>
                        <div className="data-table-pagination">
                            ...
                            <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.pd_miskin.total) ? "disabled" : "" )}>
                                <i className="icon icon-next color-gray"></i>
                            </a>
                            <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.pd_miskin.total)} PD Miskin</span>
                        </div>
                    </div>
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} colSpan="1" style={{minWidth:'40px'}}>&nbsp;</th>
                                    ...
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} rowSpan="2" style={{minWidth:'250px', color:'#434343', fontSize:'15px', textAlign:'center'}}>Nama</th>
                                    ...
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} rowSpan="2" style={{minWidth:'120px', color:'#434343', fontSize:'15px', textAlign:'center'}}>Kecamatan</th>
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} rowSpan="2" style={{minWidth:'120px', color:'#434343', fontSize:'15px', textAlign:'center'}}>Kabupaten/Kota</th>
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} rowSpan="2" style={{minWidth:'120px', color:'#434343', fontSize:'15px', textAlign:'center'}}>Provinsi</th>
                                    ...
                                    <th className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : "label-cell"} rowSpan="2" style={{minWidth:'120px', color:'#434343', fontSize:'15px', textAlign:'center'}}>Bentuk<br/>Satuan Pendidikan</th>
                            ...
                            <tbody>
                                {this.props.pd_miskin.rows.map((option)=>{
                                    return (
                                        <tr key={option.nik}>
                                            <td>
                                                <Button raised fill style={{background:((parseInt(option.is_miskin) === 1 || parseInt(option.is_miskin) === 9) ? '#cccccc' : '#007AFF'), color:((parseInt(option.is_miskin) === 1 || parseInt(option.is_miskin) === 9) ? '#434343' : '#ffffff')}} onClick={()=>this.$f7router.navigate("/verifikasiPDMiskin/"+option.nik)}>
                                                    <i className="icon f7-icons" style={{fontSize:'20px'}}>checkmark_rectangle</i>&nbsp;&nbsp;{((parseInt(option.is_miskin) === 1 || parseInt(option.is_miskin) === 9) ? 'Terverifikasi' : 'Verifikasi')}
                                                </Button>
                                            </td>
                                            <td className="numeric-cell">{option.urutan}</td>
                                            {/* <td className="numeric-cell">{(parseInt(this.props.pd_miskin.rows.indexOf(option))+1)}</td> */}
                                            <td className="label-cell">
                                                {option.miskinkah ? option.miskinkah : 'Belum diverifikasi'}
                                            </td>
                                            <td className="label-cell">
                                                {option.nama}
                                            </td>
                                            ...
                                            <td className="label-cell">
                                                {parseInt(option.bentuk_pendidikan_id) === 5 ? 'SD' : (parseInt(option.bentuk_pendidikan_id) === 6 ? 'SMP' : (parseInt(option.bentuk_pendidikan_id) === 13 ? 'SMA' : (parseInt(option.bentuk_pendidikan_id) === 15 ? 'SMK' : '-')))} - {(parseInt(option.status_sekolah) === 1 ? 'Negeri' : 'Swasta')}
                                            </td>
                                            <td className="label-cell">
                                                {option.nama_sekolah}
                                            </td>
...
export default (connect(mapStateToProps, mapDispatchToProps)(formAnakMiskin));
