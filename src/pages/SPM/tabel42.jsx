...
class tabel42 extends Component {
    state = {
        error: null,
        loading: false,
        routeParams: {
    ...
    {
        return (
            <Page name="tabel42" hideBarsOnScroll>
                <Navbar sliding={false} large backLink="Kembali">
                    <NavTitle sliding className={this.state.loading ? "skeleton-text skeleton-effect-blink" : ""}>Realisasi Pencapaian SPM</NavTitle>
                    <NavTitleLarge style={{background:'#F6BC31', color:'white'}} className={this.state.loading ? "skeleton-text skeleton-effect-blink" : ""}>Realisasi Pencapaian SPM</NavTitleLarge>
                    <NavRight>
                        <Button raised fill className="bawahCiriBiru" style={{display:'inline-flex', borderRadius:'20px'}} onClick={()=>this.$f7router.navigate('/form42/')}>
                ...
                <Block strong style={{
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <table>
                            ...
                            <tbody>
                                {this.state.tabel_42.rows.map((option)=>{
                                    return (
                                        <tr key={option.realisasi_pencapaian_pemenuhan_spm_id}>
                                            <td>
                                                <Button raised fill style={{display:'inline-flex'}} onClick={()=>this.$f7router.navigate("/form42/"+option.realisasi_pencapaian_pemenuhan_spm_id)}>
                                                    <i className="f7-icons icons" style={{fontSize:'20px'}}>pencil_circle</i>&nbsp;
                                                    Edit
                                                </Button>
                                            </td>
                                            <td>{(parseInt(this.state.tabel_42.rows.indexOf(option))+1)}</td>
...
export default (connect(mapStateToProps, mapDispatchToProps)(tabel42));
