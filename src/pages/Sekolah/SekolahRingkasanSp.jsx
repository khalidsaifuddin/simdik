...
class SekolahRingkasanSp extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapSekolahRingkasanSp(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/Sekolah/getRekapSekolahRingkasanSpExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    {
        return (
            <Page name="SekolahRingkasanSp" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={"/Sekolah/Ringkasan"}>Per Wilayah</Button>
                ...
                <Navbar sliding={false}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/Ringkasan/1/"+this.state.routeParams.kode_wilayah : "/Sekolah/Ringkasan/2/"+this.state.routeParams.kode_wilayah)}>Per Wilayah</Button>
                            <Button tabLink="#tab2" tabLinkActive>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                    ...
                    <div className="data-table" style={{overflowY:'hidden'}}>
                        <div className="data-table-footer" style={{display:'block'}}>
                            <div className="data-table-pagination">
                                ...
                                <a onClick={this.klikNext} href="#" className={"link "+((parseInt(this.state.routeParams.start)+20) > parseInt(this.props.rekap_sekolah_ringkasan_sp.total) ? "disabled" : "" )}>
                                    <i className="icon icon-next color-gray"></i>
                                </a>
                                <span className="data-table-pagination-label">{(this.state.routeParams.start+1)}-{(this.state.routeParams.start)+parseInt(this.state.routeParams.limit)} dari {this.formatAngka(this.props.rekap_sekolah_ringkasan_sp.total)} sekolah</span>
                            </div>
                        </div>
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    ...
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colspan="4">Rekapitulasi</th>
                                </tr>
                                <tr>
                            ...
                            :
                            <>
                            {this.props.rekap_sekolah_ringkasan_sp.rows.map((option)=>{
                                return(
                                    <tr>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    {/* <MenuDropdownItem href={"/Sekolah/Ringkasan/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.guru))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pegawai))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.rombel))}</td>
                                    </tr>
                                )
                            })}
                            </>
...
export default (connect(mapStateToProps, mapDispatchToProps)(SekolahRingkasanSp));