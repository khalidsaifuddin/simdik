...
class SekolahKurikulum extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapSekolahKurikulum(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/Sekolah/getRekapSekolahKurikulumExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    bukaPengaturan = () => {
        ...
        this.props.setJudulKanan('Menu Kurikulum');
    ...
    {
        return (
            <Page name="SekolahKurikulum" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            ...
                            <Button tabLink="#tab2" href={"/Sekolah/KurikulumSp/"+localStorage.getItem('id_level_wilayah_aplikasi')+"/"+localStorage.getItem('kode_wilayah_aplikasi')}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                ...
                <Navbar sliding={false}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" tabLinkActive>Per Wilayah</Button>
                            <Button tabLink="#tab2" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/KurikulumSp/1/"+this.state.routeParams.kode_wilayah : "/Sekolah/KurikulumSp/2/"+this.state.routeParams.kode_wilayah)}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                        ...
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    ...
                                    {/* <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">Kurikulum</th> */}
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">KTSP</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">K13</th>
                                </tr>
                                <tr>
                            ...
                            :
                            <>
                            {this.props.rekap_sekolah_kurikulum.rows.map((option)=>{
                                return(
                                    <tr key={option.kode_wilayah}>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    ...
                                                    <MenuDropdownItem href={"/Sekolah/Kurikulum/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
                                                        <span>Rekap Wilayah {option.nama}</span>
                                                        <Icon className="margin-left" f7="bookmark" />
                                                    </MenuDropdownItem>
                                                    }
                                                    <MenuDropdownItem href={"/Sekolah/KurikulumSp/"+(parseInt(option.id_level_wilayah))+"/"+option.kode_wilayah.trim()}>
                                        ...
                                        <td className="label-cell">
                                            {parseInt(option.id_level_wilayah) < 3 && <a href={"/Sekolah/Kurikulum/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            {parseInt(option.id_level_wilayah) === 3 && <a href={"/Sekolah/KurikulumSp/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            <br/>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.ktsp_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.ktsp_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.k13_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.k13_swasta))}</td>
                                    </tr>
                                )
                            })}
                            </>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapSekolahKurikulum: Actions.getRekapSekolahKurikulum,
...
function mapStateToProps({ App, PesertaDidik, Gtk, RaporDapodik, RekapSekolah }) {
    ...
    return {
        ...
        rekap_sekolah_kurikulum: RekapSekolah.rekap_sekolah_kurikulum,
        judul_panel_kanan: App.judul_panel_kanan,
        isi_panel_kanan: App.isi_panel_kanan
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(SekolahKurikulum));