...
class SekolahWaktuPenyelenggaraan extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
        ...
        this.setState({
            ...
            this.props.getWilayah(this.state.routeParams).then((result)=>{
                this.setState({
                    ...
                    this.props.getRekapSekolahWaktuPenyelenggaraan(this.state.routeParams).then((result)=>{
                        this.setState({
                            ...this.state,
                            loading: false
                        });
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/Sekolah/getRekapSekolahWaktuPenyelenggaraanExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    bukaPengaturan = () => {
        ...
        this.props.setJudulKanan('Menu Waktu Penyelenggaraan');
    ...
    {
        return (
            <Page name="SekolahWaktuPenyelenggaraan" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            ...
                            <Button tabLink="#tab2" href={"/Sekolah/WaktuPenyelenggaraanSp/"+localStorage.getItem('id_level_wilayah_aplikasi')+"/"+localStorage.getItem('kode_wilayah_aplikasi')}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                ...
                <Navbar sliding={false}>
                    <NavLeft >
                        <Link iconIos="f7:chevron_left" iconAurora="f7:chevron_left" iconMd="material:chevron_left" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/WaktuPenyelenggaraan" : "/Sekolah/WaktuPenyelenggaraan/1/"+this.state.routeParams.kode_wilayah.substring(0,2) + "0000" )}>Kembali</Link>
                    </NavLeft>
                    <NavTitle sliding>{this.props.wilayah.rows[0].nama}</NavTitle>
                    <NavTitleLarge>
                        Waktu Penyelenggaraan
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" tabLinkActive>Per Wilayah</Button>
                            <Button tabLink="#tab2" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/WaktuPenyelenggaraanSp/1/"+this.state.routeParams.kode_wilayah : "/Sekolah/WaktuPenyelenggaraan/2/"+this.state.routeParams.kode_wilayah)}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                        ...
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    ...
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="9">Waktu Penyelenggaraan</th>
                                </tr>
                                <tr>
                                    <th className="numeric-cell">Pagi</th>
                                    <th className="numeric-cell">Siang</th>
                                    <th className="numeric-cell">Sore</th>
                                    <th className="numeric-cell">Malam</th>
                                    <th className="numeric-cell">Kombinasi</th>
                                    <th className="numeric-cell">Sehari Penuh<br/>(5 h/m)</th>
                                    <th className="numeric-cell">Sehari Penuh<br/>(6 h/m)</th>
                                    <th className="numeric-cell">Lainnya</th>
                                    <th className="numeric-cell">Belum Isi</th>
                                </tr>
                            </thead>
                            <tbody>
                            {this.state.loading ?
                            ...
                            :
                            <>
                            {this.props.rekap_sekolah_waktu_penyelenggaraan.rows.map((option)=>{
                                return(
                                    <tr key={option.kode_wilayah}>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    ...
                                                    <MenuDropdownItem href={"/Sekolah/WaktuPenyelenggaraan/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
                                                        <span>Rekap Wilayah {option.nama}</span>
                                                        <Icon className="margin-left" f7="bookmark" />
                                                    </MenuDropdownItem>
                                                    }
                                                    <MenuDropdownItem href={"/Sekolah/WaktuPenyelenggaraanSp/"+(parseInt(option.id_level_wilayah))+"/"+option.kode_wilayah.trim()}>
                                        ...
                                        <td className="label-cell">
                                            {parseInt(option.id_level_wilayah) < 3 && <a href={"/Sekolah/WaktuPenyelenggaraan/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            {parseInt(option.id_level_wilayah) === 3 && <a href={"/Sekolah/WaktuPenyelenggaraanSp/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            <br/>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.pagi))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.siang))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.sore))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.malam))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.kombinasi))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.sehari_penuh_5))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.sehari_penuh_6))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.lainnya))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.belum_isi))}</td>
                                    </tr>
                                )
                            })}
                            </>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getRekapSekolahWaktuPenyelenggaraan: Actions.getRekapSekolahWaktuPenyelenggaraan,
...
function mapStateToProps({ App, PesertaDidik, Gtk, RaporDapodik, RekapSekolah }) {
    ...
    return {
        ...
        rekap_sekolah_waktu_penyelenggaraan: RekapSekolah.rekap_sekolah_waktu_penyelenggaraan,
        judul_panel_kanan: App.judul_panel_kanan,
        isi_panel_kanan: App.isi_panel_kanan
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(SekolahWaktuPenyelenggaraan));