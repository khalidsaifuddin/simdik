...
class SekolahAkreditasi extends Component {
    state = {
        error: null,
        loading: true,
        routeParams: {
                    ...
                    this.props.setIsiKanan((
                        ...
                        <List>  
                            <ListItem style={{cursor:'pointer'}} title="Unduh Excel" onClick={()=>{window.open(
                                    localStorage.getItem('api_base')+"/api/Sekolah/getRekapSekolahAkreditasiExcel"
                                    +"?semester_id="+localStorage.getItem('semester_id_aplikasi')
                                    +"&tahun_ajaran_id="+localStorage.getItem('semester_id_aplikasi').substring(0,4)
    ...
    bukaPengaturan = () => {
        ...
        this.props.setJudulKanan('Menu Akreditasi');
    ...
    {
        return (
            <Page name="SekolahAkreditasi" hideBarsOnScroll>
                ...
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            ...
                            <Button tabLink="#tab2" href={"/Sekolah/AkreditasiSp/"+localStorage.getItem('id_level_wilayah_aplikasi')+"/"+localStorage.getItem('kode_wilayah_aplikasi')}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                ...
                <Navbar sliding={false}>
                    <NavLeft >
                        <Link iconIos="f7:chevron_left" iconAurora="f7:chevron_left" iconMd="material:chevron_left" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/Akreditasi" : "/Sekolah/Akreditasi/1/"+this.state.routeParams.kode_wilayah.substring(0,2) + "0000" )}>Kembali</Link>
                    </NavLeft>
                    <NavTitle sliding>{this.props.wilayah.rows[0].nama}</NavTitle>
                    <NavTitleLarge>
                        Akreditasi
                    ...
                    <Subnavbar>
                        <Segmented raised>
                            <Button tabLink="#tab1" tabLinkActive>Per Wilayah</Button>
                            <Button tabLink="#tab2" href={(parseInt(this.state.routeParams.id_level_wilayah) === 1 ? "/Sekolah/AkreditasiSp/1/"+this.state.routeParams.kode_wilayah : "/Sekolah/AkreditasiSp/2/"+this.state.routeParams.kode_wilayah)}>Per Sekolah</Button>
                        </Segmented>
                    </Subnavbar>
                    <NavRight >
                        ...
                        <table>
                            <thead style={{background:'#eeeeee'}}>
                                <tr>
                                    ...
                                    {/* <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">Akreditasi</th> */}
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">A</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">B</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">C</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">Belum Terakreditasi</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">Tidak Terakreditasi</th>
                                    <th className="label-cell" style={{textAlign:'center', color:'#434343', fontSize:'15px'}} colSpan="2">Lainnya</th>
                                </tr>
                                <tr>
                            ...
                            :
                            <>
                            {this.props.rekap_sekolah_akreditasi.rows.map((option)=>{
                                return(
                                    <tr key={option.kode_wilayah}>
                                        <td className="label-cell">
                                            <MenuItem style={{marginLeft: 'auto', marginTop: '4px', marginBottom: '4px'}} iconF7="menu" dropdown className="MenuDetail">
                                                <MenuDropdown left style={{zIndex:999999}}>
                                                    ...
                                                    <MenuDropdownItem href={"/Sekolah/Akreditasi/"+option.id_level_wilayah+"/"+option.kode_wilayah} onClick={()=>this.dataRaporWilayah(option.kode_wilayah.trim())}>
                                                        <span>Rekap Wilayah {option.nama}</span>
                                                        <Icon className="margin-left" f7="bookmark" />
                                                    </MenuDropdownItem>
                                                    }
                                                    <MenuDropdownItem href={"/Sekolah/AkreditasiSp/"+(parseInt(option.id_level_wilayah))+"/"+option.kode_wilayah.trim()}>
                                        ...
                                        <td className="label-cell">
                                            {parseInt(option.id_level_wilayah) < 3 && <a href={"/Sekolah/Akreditasi/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            {parseInt(option.id_level_wilayah) === 3 && <a href={"/Sekolah/AkreditasiSp/"+option.id_level_wilayah+"/"+option.kode_wilayah}>{option.nama}</a>}
                                            <br/>
                                        ...
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.a_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.a_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.b_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.b_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.c_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.c_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.tidak_terakreditasi_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.tidak_terakreditasi_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.belum_terakreditasi_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.belum_terakreditasi_swasta))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.lainnya_negeri))}</td>
                                        <td className="numeric-cell">{this.formatAngka(parseInt(option.lainnya_swasta))}</td>
...
export default (connect(mapStateToProps, mapDispatchToProps)(SekolahAkreditasi));