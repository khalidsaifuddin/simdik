...
class CustomQueryPesertaDidik extends Component {
    state = {
        error: null,
        loading: true,
        kategori: [],
        ...
        this.setState({
            ...
            for (const key in this.state.kategori_terpilih) {
                if (this.state.kategori_terpilih.hasOwnProperty(key)) {
                    ...
                    if(element){
                        arr.nama = element.nama
                        arr.keterangan = element.keterangan
                        arr.COLUMN_NAME = element.COLUMN_NAME
                        arr.parameter = 'CONTAINS'
                        arr.parameter_value = (element.parameter_value ? '-' : element.parameter_value)

                        kategori_terpilih_arr.push(arr)
                    }
    ...
    unduhexcel = () => {
        ...
        if(this.state.custom_query.total > 1000000){
            ...
            this.props.runCustomQueryDatamart({
                ...
                this.$f7.dialog.close()
                this.JSONToCSVConvertor(result.payload.rows, 'Custom Query Peserta Didik', true)
    ...
    {
        return (
            <Page name="CustomQueryPesertaDidik" hideBarsOnScroll>
                <Navbar sliding={false} onBackClick={this.backClick}>
                    ...
                    <NavTitle sliding>Custom Query Peserta Didik</NavTitle>
                </Navbar>
...
export default (connect(mapStateToProps, mapDispatchToProps)(CustomQueryPesertaDidik));